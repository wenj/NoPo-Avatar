...
@dataclass
class LossChamfer2DCfg:
    weight: float
    single_directional: bool


@dataclass
class LossChamfer2DCfgWrapper:
    chamfer2d: LossChamfer2DCfg


class LossChamfer2D(Loss[LossChamfer2DCfg, LossChamfer2DCfgWrapper]):
    def pad(
        self,
        gaussians: Gaussians,
    ) -> tuple[
    ...
    def forward(
        ...
        for i in range(b):
            ...
            loss += chamfer_distance(
                uvs_single[indices_single > 0].unsqueeze(0),
                uvs_single[indices_single == 0].unsqueeze(0).detach(),
