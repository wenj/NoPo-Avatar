...
def train(cfg_dict: DictConfig):
    ...
    # logger.watch(encoder, log_freq=20)
    ...
    if cfg.model.encoder.pretrained_weights and cfg.mode == "train":
        ...
        if 'model' in ckpt_weights:
            ...
            current_state_dict = encoder.state_dict()
            new_ckpt_weights = {}
            for key, value in ckpt_weights.items():
                ...
                if value.shape == current_state_dict[key].shape:
                    ...
                    if key == "backbone.template_embed":
                        print("resizing template embed")
                        if cfg.model.encoder.backbone.intrinsics_embed_loc == "none":
                            original_size = int(math.sqrt(value.shape[0]))
                            new_size = int(math.sqrt(current_state_dict[key].shape[0]))
                            value_grid = value.transpose(1, 0).reshape(1, -1, original_size, original_size)
                            ...
                            new_ckpt_weights[key] = value_grid
                        else:
                            original_size = int(math.sqrt(value.shape[0] - 1))
                            new_size = int(math.sqrt(current_state_dict[key].shape[0] - 1))
                            value_grid = value[:-1].transpose(1, 0).reshape(1, -1, original_size, original_size)
                            value_grid = F.interpolate(value_grid, size=(new_size, new_size), mode="bilinear")
                            value_grid = value_grid.reshape(-1, new_size * new_size).transpose(1, 0)
