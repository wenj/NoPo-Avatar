...
class StaticModel(AutoInit):
    ...
    def __init__(self, N, f_m=None, col_n=None,
        ...
        if obj is not None:
            ...
            vt_n = None if obj['vt'] is None else obj['vt'].shape[0]
            vn_n = None if obj['vn'] is None else obj['vn'].shape[0]
            col_n = None if obj['vc'] is None else obj['vc'].shape[0]
    ...
    def from_obj(self, obj):
        ...
        if obj['vc'] is not None:
            s_vc = np.zeros((N, 3)).astype(np.float32)
            s_vc[:obj['vc'].shape[0]] = obj['vc']
            self.vc.from_numpy(s_vc)
            self.type[None] = 1
    ...
    @ti.func
    def texSample(self, coor):
        #result = self.tex is not None

        if ti.static(self.tex != None):
