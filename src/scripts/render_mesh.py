from . import taichi_three as t3
import numpy as np
...
def readply(path):
    ...
    colors = np.array([list(v)[3:6] for v in verts], dtype=np.float32) / 255.
...
def save(pid, data_id, vid, save_path, extr, intr, depth, img, mask,
    ...
    cv2.imwrite(os.path.join(img_save_path, '{}.png'.format(vid)), img)
...
def render_data(renderer, obj_path, smpl_path, texture_path, data_id, save_path, cam_nums, res,
                dis=1.0, is_thuman=False):
    os.makedirs(os.path.join(save_path, 'aux', data_id), exist_ok=True)
    obj = readply(os.path.join(obj_path, data_id, f'{data_id}.ply'))

    # smpl_ply = trimesh.load(smpl_path)
    # smpl_ply.export(smpl_path.replace('.ply', '.obj'))
    # smpl_obj_path = smpl_path.replace('.ply', '.obj')
    # texture = cv2.imread(texture_path)[:, :, ::-1]
    ...
    offset = [np.mean(obj['vi'][:, 0]), np.min(obj['vi'][:, 1]), np.mean(obj['vi'][:, 2])]
    obj['vi'] -= np.array(offset)[None]
    print(offset)
    ...
    output_obj_path = os.path.join(save_path, 'aux', data_id,
    ...
    output_transform_path = output_obj_path = os.path.join(save_path, 'aux', data_id, '%s_transform.npy' % data_id)
    # output_smpl_obj_path = os.path.join(save_path, 'aux', data_id, '%s_smplx_modified.obj'% data_id)
    # output_original_smpl_obj_path = os.path.join(save_path, 'aux', data_id, '%s_smplx.obj' % data_id)
    ...
    if is_thuman:
        ...
        smpl_path = os.path.join(smpl_path.replace('.ply', '.pkl'))
        with open(smpl_path, 'rb') as f:
            smpl_para = pickle.load(f)

        y_orient = smpl_para['global_orient'][1]
    ...
    for pid in range(cam_nums):
        ...
        extr, intr, depth, img, mask = render(dis, angle1,
                                                      look_at_center,
                                                      base_cam_pitch, renderer,
                                                      render_2k=False)
        save(pid, data_id, 2, save_path, extr, intr, depth, img, mask)
        extr, intr, depth, img, mask = render(dis, angle2,
                                                      look_at_center,
                                                      base_cam_pitch, renderer,
                                                      render_2k=False)
        save(pid, data_id, 3, save_path, extr, intr, depth, img, mask)
        extr, intr, depth, img, mask = render(dis, angle3,
                                                      look_at_center,
                                                      base_cam_pitch, renderer,
                                                      render_2k=False)
...
if __name__ == '__main__':
    ...
    scene_radius = 2.0
    res = (1024, 1024)
    smplx_root = '/home/jw116/vol23/data/2K2K/raw/smplx'
    thuman_root = '/home/jw116/vol23/data/2K2K/raw/train/1M'
    source_root = '/home/jw116/vol23/data/2K2K'
    save_root = '/home/jw116/vol23/data/2K2K/rendered'
    ...
    for phase in ['train',]:
        ...
        split_file = os.path.join(source_root,'{}.txt'.format(phase))
        thuman_list = []
        ...
        for data_id in tqdm(thuman_list):
            render_data(renderer, thuman_root, smplx_root, None, data_id,
                        save_root,
                        cam_nums, res, dis=scene_radius, is_thuman=False)