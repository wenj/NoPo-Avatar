...
class Gaussians:
    ...
    lbs_weights: Float[Tensor, "*batch dim"]


@dataclass
class GaussianLBSAdapterCfg:
    gaussian_scale_min: float
    gaussian_scale_max: float
    sh_degree: int


class GaussianLBSAdapter(nn.Module):
    cfg: GaussianLBSAdapterCfg

    def __init__(self, cfg: GaussianLBSAdapterCfg):
        super().__init__()
        self.cfg = cfg
    ...
    @property
    def d_in(self) -> int:
        return 7 + 3 * self.d_sh + 55


class UnifiedGaussianLBSAdapter(GaussianLBSAdapter):
    def forward(
        self,
        means: Float[Tensor, "*#batch 3"],
        depths: Float[Tensor, "*#batch"],
