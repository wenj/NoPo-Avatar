...
class AsymmetricCroCoMulti(CroCoNet):
    ...
    def _decoder(self, feat, pose, extra_embed=None):
        ...
        def layer(blk1, blk2, feat_current, pose, pos_ctx):
            feat_current_ctx = generate_ctx_views(feat_current)
            # img1 side
            f1, _ = blk1(feat_current[:, 0].contiguous(), feat_current_ctx[:, 0].contiguous(), pose[:, 0].contiguous(),
                         pos_ctx[:, 0].contiguous())
            f1 = f1.unsqueeze(1)
            # img2 side
            f2, _ = blk2(rearrange(feat_current[:, 1:].contiguous(), "b v l c -> (b v) l c").contiguous(),
                         rearrange(feat_current_ctx[:, 1:].contiguous(), "b v l c -> (b v) l c").contiguous(),
                         rearrange(pose[:, 1:].contiguous(), "b v l c -> (b v) l c").contiguous(),
                         rearrange(pos_ctx[:, 1:].contiguous(), "b v l c -> (b v) l c").contiguous())
            f2 = rearrange(f2, "(b v) l c -> b v l c", b=b, v=v - 1).contiguous()
            return f1, f2
        ...
        for blk1, blk2 in zip(self.dec_blocks, self.dec_blocks2):
            ...
            if l > 4000:
                f1, f2 = checkpoint(layer, blk1, blk2, feat_current, pose, pos_ctx, use_reentrant=False)
            else:
                f1, f2 = layer(blk1, blk2, feat_current, pose, pos_ctx)
            # store the result
            final_output.append(torch.cat((f1, f2), dim=1))

        # normalize last output
        del final_output[1]  # duplicate with final_output[0]
        last_feat = rearrange(final_output[-1], "b v l c -> (b v) l c").contiguous()
        last_feat = self.dec_norm(last_feat)
        final_output[-1] = rearrange(last_feat, "(b v) l c -> b v l c", b=b, v=v).contiguous()
        return final_output
