...
class DPTOutputAdapter_fix(DPTOutputAdapter):
    ...
    def forward(self, encoder_tokens: List[torch.Tensor], depths, imgs, image_size=None, conf=None, return_feat=False):
        ...
        H, W = image_size
        # Number of patches in height and width
        N_H = H * 2 // (self.stride_level * self.P_H)
        N_W = W * 2 // (self.stride_level * self.P_W)
        ...
        # Fuse layers using refinement stages
        path_4 = self.scratch.refinenet4(layers[3])[:, :, :layers[2].shape[2], :layers[2].shape[3]] # 1/16
        path_3 = self.scratch.refinenet3(path_4, layers[2]) # 1/8
        path_2 = self.scratch.refinenet2(path_3, layers[1]) # 1/4
        ...
        # path_1 = self.feat_up(path_1)
        # print(path_1.shape, direct_img_feat.shape)
...
def create_gs_dpt_head_debug(net, has_conf=False, out_nchan=3, postprocess_func=postprocess):
    """
