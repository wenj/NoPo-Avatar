...
tmp_ours = './tmp/eval_thuman/pred'
tmp_gt = './tmp/eval_thuman/gt'
tmperr = './tmp/eval_thuman/error'
...
def func(ref_path, g_path, t_path, mask):
    ...
    for human_idx in range(len(humans)):
        ...
        for i, angle in enumerate([3, 8, 13]):

            sample_name = '{}_{}_novel00'.format(human, str(angle).zfill(3))
            print(sample_name)

            ours_path = os.path.join(g_path, '{}/color/{:06d}.png'.format(human, i + 3))
            g = imageio.imread(ours_path).astype('float32') / 255.

            gt_mask = os.path.join(mask.format(human), 'frame_{:06d}.png'.format(i + 3))
            gt_mask = imageio.imread(gt_mask).astype('float32') / 255.

            gt_path = os.path.join(t_path.format(human), 'frame_{:06d}.png'.format(i + 3))
...
def evaluateErr(ref, ours, target, mask):
    print(ref, ours, target, mask)

    psnrs, ssims, mses, maes = func(ref_path=ref, g_path=ours, t_path=target, mask=mask)
...
# path to GT rgb
target = 'datasets/thuman/val'
# path to GT mask
mask = 'datasets/thuman/val/{}/masks'
# path to prediction
ours = 'outputs/test/train_thuman2.0_3views_res1024'
print(ours)

# XHuman
# # path to GT rgb
# target = './datasets/XHuman/val/img/'
# # path to GT mask
# mask = './datasets/XHuman/val/mask/'
# # path to prediction
# ours = '/u/jwen/codes/generalizable_point-based-human/log/ghg_view3_subdivide_deepf_pointtransformer_iter3_fb_tf_lpips0.5_adam_lr1e-4_5e-5_nolpips_ssim_coeff1.0_lap100.0/eval/view_xhuman'
...
evaluateErr(target, ours, os.path.join(target, "{}", "images"), mask)
