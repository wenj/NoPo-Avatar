...
tmp_ours = './tmp/eval_xhuman_nps/pred'
tmp_gt = './tmp/eval_xhuman_nps/gt'
tmperr = './tmp/eval_xhuman_nps/error'
...
def func(ref_path, g_path, t_path, mask):
    ...
    for human_idx in range(len(humans)):
        ...
        for i, angle in enumerate([3, 8, 13]):
            ...
            gt_mask = os.path.join(mask, human, 'masks', 'frame_{}.png'.format(str(i + 3).zfill(6)))
            gt_mask = imageio.imread(gt_mask).astype('float32') / 255.

            gt_path = os.path.join(mask, human, 'images', 'frame_{}.png'.format(str(i + 3).zfill(6)))
...
# path to GT rgb
target = 'datasets/XHuman/val-nps-multilhm'
# path to GT mask
mask = 'datasets/XHuman/val-nps-multilhm'
# path to prediction
ours = 'outputs/test/train_thuman2.0_3views_res1024_xhuman_nps'
...
evaluateErr(target, ours, target, mask)
